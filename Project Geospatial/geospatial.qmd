...
# ── 0. Build panel ─────────────────────────────────────────────────────────────
panel <- panel_filtered %>%          # use the deduplicated panel (max PGA)
  left_join(                         # re-attach event name from pga_all
    panel %>%                        # original panel still has event column
      select(state, municipality, year, quarter, event) %>%
      filter(!is.na(event)),
    by = c("state", "municipality", "year", "quarter")
  ) %>%
