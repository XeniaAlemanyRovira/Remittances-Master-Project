...
# Libraries and partitions already loaded in panel-prep chunk above
...
run_cs_partition <- function(data, pga_lo, pga_hi, partition_label,
                              ...
                              post_window =  20) { # post-periods for event study
  ...
  # FIX: aggte(type="simple") silently ignores min_e/max_e in did < 2.1.2,
  # averaging over ALL (g,t) cells including pre-treatment (≈0), which dilutes
  # the post-treatment ATT toward zero. Instead, we use aggte(type="dynamic")
  # which reliably respects the event-time window, then manually compute the
  # overall post-treatment ATT from the event-study coefficients at e >= 0.
  es <- tryCatch(
    aggte(cs_out,
          type  = "dynamic",
          min_e = pre_window,
  ...
  # Compute post-treatment ATT manually from dynamic estimates at e >= 0
  agg <- NULL
  if (!is.null(es)) {
    post_idx <- which(es$egt >= 0 & es$egt <= post_window)
    if (length(post_idx) > 0) {
      post_atts <- es$att.egt[post_idx]
      # Simple average of post-treatment event-time coefficients
      post_att  <- mean(post_atts, na.rm = TRUE)
      # SE via delta method (conservative: assume independence across e)
